...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.pixel-caf.projectname)
    if [ -z "$REMOTE" ]
    then
...
Commands:
    ...
    push    Push current branch to Gerrit
...
Example:
    $FUNCNAME push
will push current branch to Gerrit for branch '$remote_branch'.
EOF
                    ;;
                *)
...
function pixelrebase() {
    ...
    cd $dir
    repo=$(git config --get remote.pixel-caf.projectname)
    if [ -z "$repo" ]
    then
